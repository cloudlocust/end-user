...
export const AnalysisMaxPower = ({ data, housingId }: AnalysisMaxPowerProps) => {
    ...
    const pmaxMetric = useMemo(() => data.filter((elem) => elem.target === metricTargetsEnum.pMax)[0], [data])
    ...
    useEffect(() => {
        if (pmaxMetric) {
            const { maxValue, maxValueTimestamp, unit } = conputePMaxWithTimestamp(pmaxMetric)
            setPmaxValue(maxValue)
            setPmaxTimestamp(maxValueTimestamp)
            setPmaxUnit(unit)
        }
    }, [pmaxMetric])
