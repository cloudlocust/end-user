...
describe('EcogesteHook test', () => {
    test('useEcogestes should return all ecogestes filtered using categoryId', async () => {
        const {
            renderedHook: { result, waitForValueToChange },
        } = reduxedRenderHook(() => useEcogestes(false), { initialState: {} })
    ...
    test('useEcogestes should return just viewed ecogestes filtered using categoryId', async () => {
        const {
            renderedHook: { result, waitForValueToChange },
        } = reduxedRenderHook(() => useEcogestes(true), { initialState: {} })
        ...
        expect(result.current.loadingInProgress).toBe(false)
        expect(result.current.elementList).toBeTruthy()
        expect(result.current.elementList.length).toBe(1)
    })
    ...
    describe('Patch Ecogeste', () => {
        test('patchEcogeste with correct ID and payload returns 200', async () => {
            const {
                renderedHook: { result, waitForValueToChange },
            } = reduxedRenderHook(() => useEcogestes(false), {
                initialState: {},
            })
